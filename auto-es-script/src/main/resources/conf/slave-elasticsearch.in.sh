...
export LOGDIR=/opt/logs/elasticsearch
export ES_HEAP_SIZE="1g"
export JAVA_OPTS="-Xmn1g -XX:TargetSurvivorRatio=85 -Xss256k -XX:+PrintGCDateStamps "
...
if [ "x$ES_GC_OPTS" = "x" ]; then
  ...
  ES_GC_OPTS="$ES_GC_OPTS -XX:CMSInitiatingOccupancyFraction=85"
...
if [ -n "$ES_GC_LOG_FILE" ]; then
  ...
  JAVA_OPTS="$JAVA_OPTS -Xloggc:$LOGDIR/es.gc.log"
...
JAVA_OPTS="$JAVA_OPTS -XX:HeapDumpPath=$LOGDIR/heapdump.hprof"
